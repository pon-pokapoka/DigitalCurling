...
SimulatorFCV1Light::SimulatorFCV1Light(SimulatorFCV1LightFactory const& factory)
    : SimulatorFCV1Light(SimulatorFCV1LightStorage(factory))
{}

SimulatorFCV1Light::SimulatorFCV1Light(SimulatorFCV1LightStorage const& storage)
    : storage_(storage)
    , world_(b2Vec2_zero)
    , stone_bodies_()
...
void SimulatorFCV1Light::SetStones(ISimulator::AllStones const& stones)
{
    // update bodies
    for (int i = 0; i < StoneCoordinate::kStoneMax; ++i) {
        if (stones[i]) {
...
void SimulatorFCV1Light::AdaptiveStep(float seconds_per_frame)
{
    // simulate
    for (auto stone_body : stone_bodies_) {
        ...
        if (stone_speed > std::numeric_limits<float>::epsilon()) {
            ...
            float const new_stone_speed = stone_speed + longitudinal_acceleration * seconds_per_frame;
            if (new_stone_speed <= 0.f) {
                ...
                float const yaw = yaw_rate() * seconds_per_frame;
        ...
        if (std::abs(angular_velocity) > std::numeric_limits<float>::epsilon()) {
            float const angular_accel = -0.025f / std::max(stone_speed, 0.001f) * seconds_per_frame;
...
ISimulator::AllStones const& SimulatorFCV1Light::GetStones() const
{
    if (stones_dirty_) {
        // update stones_
        for (int i = 0; i < StoneCoordinate::kStoneMax; ++i) {
...
std::vector<ISimulator::Collision> const& SimulatorFCV1Light::GetCollisions() const
{
    return storage_.collisions;
}

bool SimulatorFCV1Light::AreAllStonesStopped() const
{
...
float SimulatorFCV1Light::GetSecondsPerFrame() const
{
    return storage_.factory.seconds_per_frame;
}

ISimulatorFactory const& SimulatorFCV1Light::GetFactory() const
{
    return storage_.factory;
}

std::unique_ptr<ISimulatorStorage> SimulatorFCV1Light::CreateStorage() const
{
    GetStones(); // Box2D側のデータを AllStones に適用する
    return std::make_unique<SimulatorFCV1LightStorage>(storage_);
}

void SimulatorFCV1Light::Save(ISimulatorStorage & storage) const
{
    GetStones(); // Box2D側のデータを AllStones に適用する
    static_cast<SimulatorFCV1LightStorage &>(storage) = storage_;
}

void SimulatorFCV1Light::Load(ISimulatorStorage const& storage)
{
    storage_ = static_cast<SimulatorFCV1LightStorage const&>(storage);
    UpdateWithStorage();
}

moves::Shot SimulatorFCV1Light::CalculateShot(Vector2 const& target_position, float const target_speed, float const shot_angular_velocity) const {
    if (target_speed < 0.f)
        throw std::invalid_argument("SimulatorFCV1Light::CalculateShot: target_speed must be non-negative.");
    if (target_speed > 4.f)
        throw std::invalid_argument("SimulatorFCV1Light::CalculateShot: target_speed is too large.");

    auto const target_r = target_position.Length();
    if (target_r < 0.1f)
        throw std::invalid_argument("SimulatorFCV1Light::CalculateShot: target_position is too close to the origin.");
    ...
    if (target_speed >= v0_speed)
        throw std::invalid_argument("SimulatorFCV1Light::CalculateShot: target_speed is too large for the target_position.");
    ...
    Vector2 const delta = [angular_velocity, v0_speed, target_speed] {
        ...
        if (!s_simulator)
            s_simulator = simulators::SimulatorFCV1LightFactory().CreateSimulator();
...
void SimulatorFCV1Light::ContactListener::PostSolve(b2Contact* contact, const b2ContactImpulse* impulse)
{
...
void SimulatorFCV1Light::UpdateWithStorage()
{
